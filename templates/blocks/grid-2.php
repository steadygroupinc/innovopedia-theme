...
if ( ! function_exists( 'foxiz_get_grid_2' ) ) {
	 ...
	 * @return false|string
	 */
	function foxiz_get_grid_2( $settings = [], $_query = null ) {
		...
		$settings = foxiz_get_design_standard_block( $settings, 'grid_2' );
		...
		if ( ! $_query->have_posts() ) {
			...
			foxiz_loop_grid_2( $settings, $_query );
...
if ( ! function_exists( 'foxiz_loop_grid_2' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function foxiz_loop_grid_2( $settings, $_query ) {
