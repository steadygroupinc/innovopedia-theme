...
if ( ! function_exists( 'foxiz_get_list_box_2' ) ) {
	function foxiz_get_list_box_2( $settings = [], $_query = null ) {
		...
		$settings = foxiz_get_design_standard_block( $settings, 'list_box_2' );
		...
		if ( ! $_query->have_posts() || $_query->post_count < $min_posts ) {
			...
			foxiz_loop_list_box_2( $settings, $_query );
...
if ( ! function_exists( 'foxiz_loop_list_box_2' ) ) {
	function foxiz_loop_list_box_2( $settings, $_query ) {
