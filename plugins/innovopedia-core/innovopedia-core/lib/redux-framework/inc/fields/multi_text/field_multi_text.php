...
if ( ! class_exists( 'ReduxFramework_multi_text' ) ) {
    ...
    class ReduxFramework_multi_text {
         ...
         * @return      void
         */
	    public $parent, $field, $value, $add_text, $show_empty;
        ...
        public function render() {
            ...
            if ( isset( $this->value ) && is_array( $this->value ) ) {
                foreach ( $this->value as $k => $value ) {
                    if ( $value != '' || ($value == '' && $this->show_empty == true) ) {
                        echo '<li>';
                        echo     '<input readonly type="text" id="' . $this->field['id'] . '-' . $k . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[]' . '" value="' . esc_attr( $value ) . '" class="regular-text ' . $this->field['class'] . '" /> ';
	        ...
	        if ( ! empty( $this->field['label'] ) ) {
		        echo '<span class="repeat-label">' . esc_html( $this->field['label'] ) . '</span>';
	        }
	        echo '<input type="text" id="' . $this->field['id'] . '" name="' . $the_name . '" value="" class="regular-text" /> ';
	        echo '<a' . ' data-id="' . $this->field['id'] . '-ul" href="javascript:void(0);" class="deletion redux-multi-text-remove">' . __( 'Remove', 'redux-framework' ) . '</a>';
	        echo '</li>';
	        echo '</ul>';
